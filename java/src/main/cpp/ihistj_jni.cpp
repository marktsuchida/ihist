...
// Validate common parameters for 8-bit histograms
auto validate_params8(JNIEnv *env, jint sample_bits, jint height, jint width,
                      jint image_stride, jint mask_stride, jint n_components,
                      jintArray component_indices) -> bool {
    if (sample_bits < 1 || sample_bits > 8) {
...
// Validate common parameters for 16-bit histograms
auto validate_params16(JNIEnv *env, jint sample_bits, jint height, jint width,
                       jint image_stride, jint mask_stride, jint n_components,
                       jintArray component_indices) -> bool {
    if (sample_bits < 1 || sample_bits > 16) {
        throw_illegal_argument(
            env, "sampleBits must be in range [1, 16] for 16-bit");
        return false;
    }
    if (height < 0 || width < 0) {
        throw_illegal_argument(env, "height and width must be non-negative");
...
// Get buffer position for NIO buffers
...
// Get buffer remaining for NIO buffers
...
// Check if buffer has backing array
...
Java_ihistj_IHistNative_histogram8__I_3BI_3BIIIIII_3I_3IIZ(
    ...
    if (!validate_params8(env, sample_bits, height, width, image_stride,
                          mask_stride, n_components, component_indices)) {
        return;
    }
    ...
    // Validate array bounds for image
    if (image == nullptr) {
    ...
    // Validate array bounds for mask
    if (mask != nullptr) {
    ...
    // Validate histogram array bounds
    ...
    // Get array pointers using GetPrimitiveArrayCritical for best performance
    jbyte *image_ptr =
        static_cast<jbyte *>(env->GetPrimitiveArrayCritical(image, nullptr));
    ...
    if (mask != nullptr) {
        ...
        if (mask_ptr == nullptr) {
            env->ReleasePrimitiveArrayCritical(image, image_ptr, JNI_ABORT);
            return;
        }
    }
    ...
    // Call the C API
    // Note: Java byte is signed, but bit patterns are same as unsigned.
    // The C code treats pixels as unsigned, which is correct.
    ihist_hist8_2d(
        static_cast<std::size_t>(sample_bits),
        reinterpret_cast<std::uint8_t const *>(image_ptr + image_offset),
        mask_ptr != nullptr
    ...
    // Release arrays - commit histogram changes (mode 0)
...
Java_ihistj_IHistNative_histogram16__I_3SI_3BIIIIII_3I_3IIZ(
    ...
    if (!validate_params16(env, sample_bits, height, width, image_stride,
                           mask_stride, n_components, component_indices)) {
        return;
    }
    ...
    // Validate array bounds for image (element count, not byte count)
    if (image == nullptr) {
        throw_illegal_argument(env, "image cannot be null");
        return;
    }
    ...
    // Get array pointers
    jshort *image_ptr =
        static_cast<jshort *>(env->GetPrimitiveArrayCritical(image, nullptr));
    if (image_ptr == nullptr) {
        return;
    }
    ...
    ihist_hist16_2d(
        static_cast<std::size_t>(sample_bits),
        reinterpret_cast<std::uint16_t const *>(image_ptr + image_offset),
        mask_ptr != nullptr
            ? reinterpret_cast<std::uint8_t const *>(mask_ptr + mask_offset)
            : nullptr,
        static_cast<std::size_t>(height), static_cast<std::size_t>(width),
...
Java_ihistj_IHistNative_histogram8__ILjava_nio_ByteBuffer_2Ljava_nio_ByteBuffer_2IIIII_3ILjava_nio_IntBuffer_2Z(
    ...
    // Get image data
    std::uint8_t const *image_ptr = nullptr;
    ...
    if (image_direct != nullptr) {
        ...
        image_ptr = static_cast<std::uint8_t const *>(image_direct) + position;
    } else if (buffer_has_array(env, image_buffer)) {
        // Heap buffer with backing array
        jclass byte_buffer_class = env->GetObjectClass(image_buffer);
    ...
    if (mask_buffer != nullptr) {
        ...
        if (mask_direct != nullptr) {
            ...
            mask_ptr =
                static_cast<std::uint8_t const *>(mask_direct) + position;
        } else if (buffer_has_array(env, mask_buffer)) {
            ...
            mask_copy.assign(
                ...
                reinterpret_cast<std::uint8_t *>(arr_ptr + offset + position +
                                                 remaining));
            env->ReleaseByteArrayElements(arr, arr_ptr, JNI_ABORT);
            mask_ptr = mask_copy.data();
        } else {
            throw_illegal_argument(
                env, "mask ByteBuffer must be direct or array-backed");
            return;
        }
    }
    ...
    // Call the C API
    ihist_hist8_2d(static_cast<std::size_t>(sample_bits), image_ptr, mask_ptr,
    ...
    // Release histogram backing array if used
    if (histogram_backing_ptr != nullptr) {
...
Java_ihistj_IHistNative_histogram16__ILjava_nio_ShortBuffer_2Ljava_nio_ByteBuffer_2IIIII_3ILjava_nio_IntBuffer_2Z(
    ...
    // Get image data (ShortBuffer)
    std::uint16_t const *image_ptr = nullptr;
    std::vector<std::uint16_t> image_copy;
    void *image_direct = env->GetDirectBufferAddress(image_buffer);

    if (image_direct != nullptr) {
        jint const position = get_buffer_position(env, image_buffer);
        image_ptr = reinterpret_cast<std::uint16_t const *>(
            static_cast<char *>(image_direct) + position * sizeof(jshort));
    } else if (buffer_has_array(env, image_buffer)) {
        jclass short_buffer_class = env->GetObjectClass(image_buffer);
        jmethodID array_method =
            env->GetMethodID(short_buffer_class, "array", "()[S");
        jshortArray arr = static_cast<jshortArray>(
            env->CallObjectMethod(image_buffer, array_method));
        jint const offset = get_buffer_array_offset(env, image_buffer);
        jint const position = get_buffer_position(env, image_buffer);
        jint const remaining = get_buffer_remaining(env, image_buffer);

        jshort *arr_ptr = env->GetShortArrayElements(arr, nullptr);
        if (arr_ptr == nullptr) {
            return;
        }
        image_copy.assign(
            reinterpret_cast<std::uint16_t *>(arr_ptr + offset + position),
            reinterpret_cast<std::uint16_t *>(arr_ptr + offset + position +
                                              remaining));
        env->ReleaseShortArrayElements(arr, arr_ptr, JNI_ABORT);
        image_ptr = image_copy.data();
    } else {
        throw_illegal_argument(env,
    ...
    // Get mask data (optional, ByteBuffer)
    std::uint8_t const *mask_ptr = nullptr;
    std::vector<std::uint8_t> mask_copy;
    if (mask_buffer != nullptr) {
        void *mask_direct = env->GetDirectBufferAddress(mask_buffer);
    ...
    // Call the C API
    ihist_hist16_2d(static_cast<std::size_t>(sample_bits), image_ptr, mask_ptr,
                    static_cast<std::size_t>(height),
                    static_cast<std::size_t>(width),
                    static_cast<std::size_t>(image_stride),
                    static_cast<std::size_t>(mask_stride),
